...
struct task_struct
    *task; /*    Structure defined in sched.h for tasks/processes    */
struct task_struct
    *task_child; /*    Structure needed to iterate through task children    */
struct list_head *list; /*    Structure needed to iterate through the list in
                           each task->children struct    */
MODULE_LICENSE("GPL");

static loff_t listprocess_device_lseek(struct file *file, loff_t offset,
                                       int orig);
...
static ssize_t listprocess_read(struct file *file, char *buf, size_t size,
                                loff_t *offset);
...
static ssize_t listprocess_read(struct file *file, char *buf, size_t size,
  ...
  for_each_process(task) { /*    for_each_process() MACRO for iterating through
    ...
    list_for_each(list, &task->children) { /*    list_for_each MACRO to iterate
                                              through task->children    */

      task_child = list_entry(list, struct task_struct,
                              sibling); /*    using list_entry to declare all
                                           vars in task_child struct    */
...
static int __init init_listprocess_dev(void) {
  ...
  rc = alloc_chrdev_region(
      &listprocess_dev, 0, 1,
      DEV_LISTPROCESS_NAME); // register device number and name
  if (rc < 0) {
  ...
  if (!device_create(listprocess_class, NULL, listprocess_dev, NULL,
                     DEV_LISTPROCESS_NAME)) {
